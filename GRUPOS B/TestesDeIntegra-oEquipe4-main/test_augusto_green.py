...
class TestAugustoRelatorios(unittest.TestCase):
    ...
    # TU 7: Formatação do Relatório (Estrutura)
    ...
    # TU 8: Geração de Relatório Vazio (Sem Dados)
    ...
    # TC 4: Contrato de Saída do Relatório
    def test_contrato_schema_saida_livros_mais_emprestados(self):
        # Mock para simular a saída de um relatório
        relatorio_mock = self.service.gerar_relatorio_livros_mais_emprestados()
        
        if relatorio_mock:
            # Verifica o primeiro item, assumindo que todos seguirão o mesmo contrato
